...
parser.add_argument('-indirectory', type=str, help='Input directory holding BED files to process, in comma delimited format', nargs=1)
parser.add_argument('-outdirectory', type=str, help='Input directory to hold processed BED files, in comma delimited format')
...
directory = args['indirectory'][0]
output_directory = args['outdirectory']
...
class Peak():
    ...
    def merge(self, peak_to_merge_with, sequence_length): 
        ...
        self.extend_coords(new_merge_midpt, sequence_length)


# merge the peaks, in each chromosome, so that they are 600 bp long. followed procedure from the paper 
def extend_peaks(file_dict, row_dict, sequence_length):
    ...
    for chromosome, all_rows in file_dict.items():
        ...
        sorted_peaks_list = [Peak(interval[0], interval[1], all_rows[(interval[0], interval[1])][0], all_rows[(interval[0], interval[1])][1]) for interval in sorted_intervals]
...
def greedily_merge_peaks(max_overlap, row_dict, sequence_length):
    ...
    for chromosome in row_dict.keys():
        ...
        while i < len(all_peaks)-1:  
            ...
            if all_peaks[i].end - all_peaks[i+1].start > max_overlap: 
                ...
                all_peaks[i+1].merge(peak_to_merge_with=all_peaks[i], sequence_length=sequence_length)
    ...
    with open(concat_path, 'w') as bed_file: 
        for chrom, rows in row_dict.items(): 
            for peak in rows: 
                ...
                bed_line = chrom + ' ' + chromStart + ' ' + chromEnd + ' ' + readCount
...
def main(directory, output_directory, sequence_length):
    if os.path.exists(directory) and os.path.isdir(directory):
        ...
        for (file, file_dict) in all_files: 
            ...
            row_dict = extend_peaks(file_dict, {}, sequence_length)
            row_dict = greedily_merge_peaks(max_overlap, row_dict, sequence_length)
